...
/*
			stm.showEditorFoldout = EditorGUILayout.Foldout(stm.showEditorFoldout, "Editor", foldoutStyle);
			if(stm.showEditorFoldout)
			{
				EditorGUILayout.BeginHorizontal();
...
public class SuperTextMesh : MonoBehaviour, ILayoutElement { //MaskableGraphic... rip
	...
	public bool showEditorFoldout = false;
	...
	private static void AttachToCanvas(Transform thisTransform, MenuCommand menuCommand)
	{
		...
		if(myCanvas == null)
		{
			...
			myCanvas = myParent.GetComponentInParent<Canvas>();
		}
...
//								Debug.Log("Default wave isn't defined!");
					...
					//Audio Settings
						...
						//unicode
						case "<u=":
							insertAfter = char.ConvertFromUtf32(int.Parse(myString, System.Globalization.NumberStyles.HexNumber));
							break;
					//Default
						default:
...
*/
		...
		float maxHeight = OffsetData_VerticalLimit > 0f ? -OffsetData_VerticalLimit : rawBottomRightTextBounds.y;
		...
		rawBottomRightBounds = new Vector3(AutoWrap > 0f ? offset.x - AutoWrap : offset.x - rawBottomRightTextBounds.x, 
											OffsetData_VerticalLimit > 0f ? OffsetData_VerticalLimit + offset.y : offset.y - maxHeight, 
											offset.z);
